...
const NodeCard: React.FC<{
  ...
  >
    ...
    <text x={x + 15} y={y - 7} fill="#767676" fontFamily="JetBrains Mono, monospace" fontSize="8.5" letterSpacing="1.6">
      {tag}
    </text>
    <text x={x + 15} y={y + 12} fill="#0A0A0A" fontFamily="Inter, sans-serif" fontSize="13" fontWeight="500">
      {label}
    </text>
  </motion.g>
);
  ...
  <>
    {wires.map((wire, i) => (
      <g key={wire.id}>
        ...
        />
        {!reduced && (
          <circle r={3.4} fill={wire.out ? '#2E9BFF' : '#DC2B26'}>
...
const AgentNode: React.FC<{ cx: number; top: number; clipId: string }> = ({ cx, top, clipId }) => {
  ...
  return (
    ...
    >
      ...
      <text x={cx} y={top + 155} textAnchor="middle" fill="#0A0A0A" fontFamily="Inter, sans-serif" fontSize="13.5" fontWeight="600">
        Akshay
      </text>
    </motion.g>
  );
...
const Defs = () => (
  <defs>
    <radialGradient id="agentGlow" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stopColor="#DC2B26" stopOpacity="0.28" />
      <stop offset="100%" stopColor="#DC2B26" stopOpacity="0" />
    </radialGradient>
  </defs>
);
    ...
    <div className={`panel overflow-hidden ticked ${className}`}>
      ...
      <svg viewBox="0 0 600 420" className="hidden h-auto w-full sm:block" role="img" aria-label={label}>
        ...
        {[
          ...
          <NodeCard key={t.label} x={12} y={t.y} w={114} label={t.label} tag="TRIGGER" accent="#DC2B26" delay={0.15 + i * 0.1} />
        ))}
      ...
      <svg viewBox="0 0 360 500" className="h-auto w-full sm:hidden" role="img" aria-label={label}>
        ...
        <NodeCard x={100} y={67} w={160} label="New lead arrives" tag="TRIGGER" accent="#DC2B26" delay={0.15} />
